...
const ForgotPassword = () => {
  ...
  const sendOtp = async () => {
    ...
    try {
      const response = await axios.post("https://vibrant-vibe-cases.onrender.com/api/auth/send-otp", { email });
  ...
  const changePassword = async () => {
    setErrors({});
    try {
      await axios.post("https://vibrant-vibe-cases.onrender.com/api/auth/update-password", { email, newPassword });
  ...
  return (
    ...
    >
      <Box
        sx={{
          backgroundColor: "rgba(209, 193, 103, 0.7)",
          padding: "30px",
          borderRadius: "10px",
...
<TextField
  ...
  helperText={errors.newPassword || "8+ chars, 1 uppercase, 1 lowercase, 1 number, 1 special char"}
  InputProps={{
    endAdornment: (
